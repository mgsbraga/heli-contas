...
export function montarResumo(ledger) {
  const acc = (chave) => {
    const m = new Map();
...
export async function build({ verboso = true } = {}) {
  ...
  // 1. ledger completo
  const completo = { ...ledger, escopo: 'total', geradoEm: new Date().toISOString() };
  fs.writeFileSync(path.join(P.dados, 'ledger.enc'), JSON.stringify(await cifrar(kFull, paraBytes(completo))));

  // 2. resumo
  fs.writeFileSync(path.join(P.dados, 'resumo.enc'), JSON.stringify(await cifrar(kResumo, paraBytes(montarResumo(ledger)))));
